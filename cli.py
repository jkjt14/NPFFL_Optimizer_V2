...
def _demo_pool() -> pd.DataFrame:
    """Return a synthetic player pool to satisfy roster constraints."""
    rows = []
    # QB
    rows += [
    ...
    for i,(sal,ry,rt,rec,ryd) in enumerate([
        ...
        rows.append({"player_id":f"RB{i}","name":f"Runner {i}","team":"RBX","pos":"RB","salary":sal,
                     "pass_yd":0,"pass_td":0,"rush_yd":ry,"rush_td":rt,"rec":rec,"rec_yd":ryd,"rec_td":0})
    # WR
    for i,(sal,rec,ryd,td) in enumerate([
        ...
        rows.append({"player_id":f"WR{i}","name":f"Wide {i}","team":"WRX","pos":"WR","salary":sal,
                     "pass_yd":0,"pass_td":0,"rush_yd":0,"rush_td":0,"rec":rec,"rec_yd":ryd,"rec_td":td})
    # TE
    for i,(sal,rec,ryd,td) in enumerate([(6400,6,55,1),(5200,5,40,0),(4200,3,35,0)], start=1):
        rows.append({"player_id":f"TE{i}","name":f"Tight {i}","team":"TEX","pos":"TE","salary":sal,
                     "pass_yd":0,"pass_td":0,"rush_yd":0,"rush_td":0,"rec":rec,"rec_yd":ryd,"rec_td":td})
    # DEF
    for i, sal in enumerate([4000,3600,3200], start=1):
        rows.append({
            ...
            "pos": "DEF",
            "salary": sal,
            "pass_yd": 0, "pass_td": 0, "rush_yd": 0, "rush_td": 0,
